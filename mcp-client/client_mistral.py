"""Mistral port of client_openai.py (via Mistral's OpenAI-compatible endpoint).

Identical benchmark pipeline to client.py / client_openai.py / client_gemini.py -
MCP connection, VARIANTS, plant_notes, canary, record_result, ASR, the same
system-prompt framing and "[Calling tool ...]" transcript marker - so results
are directly comparable across Anthropic / OpenAI / Gemini / Mistral.

Only the LLM layer differs: the OpenAI SDK is pointed at Mistral's OpenAI-compat
Chat Completions endpoint. Mistral's function-calling is closely OpenAI-compatible,
so the tool JSON-Schema is passed through unmodified (if a run 400s on the tool
schema, add a sanitizer like client_gemini's).

Requires: MISTRAL_API_KEY in mcp-client/.env (openai package already installed).
"""
...
# Mistral's OpenAI-compatible endpoint - lets us reuse the OpenAI SDK unchanged.
MISTRAL_BASE_URL = "https://api.mistral.ai/v1"

# Mistral flagship + small models are standard (non-reasoning) chat models, so a
# plain max_tokens budget is fine. Confirm exact IDs against the live models list.
...
def _build_system_prompt(tools: list[dict]) -> str:
    ...
    Identical framing to the other clients so providers are comparable.
    """
...
class MCPClient:
    def __init__(self):
        ...
        if not api_key:
            raise RuntimeError(
                "MISTRAL_API_KEY is not set - add it to mcp-client/.env"
            )
        # Same OpenAI SDK, pointed at Mistral's OpenAI-compatible endpoint.
    ...
    async def _available_tools(self):
        """MCP tools -> OpenAI function-tool schema. Mistral is OpenAI-compatible,
        so the MCP inputSchema (JSON Schema) maps directly onto `parameters`."""
        response = await self.session.list_tools()
        return [
            {
                "type": "function",
    ...
    async def process_query(self, query: str) -> str:
        """Process a query using a Mistral model and available tools, looping
...
async def main():
    parser = argparse.ArgumentParser(
        description="Benchmark ASR for MCP resource-poisoning injections (Mistral)."
    )
