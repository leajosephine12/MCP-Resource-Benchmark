"""Sweep driver: run test cases 1-10 x 3 OpenAI models, content variant.

Reuses the client_openai.py machinery (MCPClient, run_single_trial) by setting
its module-level config globals per (test case, model), then writes each
test_case{N}_openai.jsonl in the same header-per-model structure as
results/test_case1.jsonl:

    <model> <successes>/<n> = <asr>%
    {json trial record}
    ... (n records)
    <blank line>
    <next model> ...
...
Usage:
    ...
    python mcp-client/sweep_openai.py --n 1 --cases 1 # quick preflight
"""
...
from test_cases_sweep import TEST_CASES

import client_openai as C

MODELS = ["gpt-5.5", "gpt-5.4-nano", "gpt-5"]
VARIANT = "content"
...
async def run_case(case_no: int, n: int) -> None:
    ...
    out_path = RESULTS_DIR / f"test_case{case_no}_openai.jsonl"
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    # apply this test case's payload to the shared client_openai globals
    ...
    try:
        for model in MODELS:
            ...
            print(
                f"  -> {model} case{case_no}: {successes}/{n} = {asr:.2%}", flush=True
            )
    finally:
...
async def main():
    parser = argparse.ArgumentParser(description="OpenAI model x test-case sweep.")
